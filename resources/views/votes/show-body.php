<div class="vote-body border-bottom">
  ...
  <div class="vote-description text-muted">
    参赛作品：<span class="js-attend-user-count"><?= count($voteWorks); ?></span>
  </div>
  ...
  <span class="vote-tips left border-right text-muted">
    投票人数：<span class="js-attend-user-count"><?= $vote->getVoteUserCount(); ?></span>
  </span>

  <span class="vote-tips right text-muted">
    ...
    <?php if ($setting('vote.isOpen')) : ?>
      <?php if (!$curVoteWork) : ?>
        <a href="<?= $url->full('vote-works/new', ['voteId' => $vote['id']]) ?>">
          <button class="btn btn-success">我要报名</button>
        </a>
      <?php else : ?>
        <a href="<?= $url->full('vote-works/%s/show', ['id' => $curVoteWork['id']]) ?>">
          <button class="btn btn-success">我的作品</button>
        </a>
      <?php endif; ?>
    <?php endif; ?>
  </span>
